...
resource "google_compute_managed_ssl_certificate" "this" {
  name = "c7n-test-certificate-170-green"
...
resource "google_compute_ssl_policy" "this" {
  name            = "c7n-ssl-policy-170-green"
  profile         = "MODERN"
  min_tls_version = "TLS_1_2"
}

resource "google_compute_target_https_proxy" "this" {
  name             = "test-proxy-170-green"
...
resource "google_compute_url_map" "this" {
  name            = "url-map-target-proxy-170-green"
...
resource "google_compute_http_health_check" "this" {
  name               = "c7n-check-backend-170-green"
  request_path       = "/"
