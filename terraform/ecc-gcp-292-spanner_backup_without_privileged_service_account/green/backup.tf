...
resource "null_resource" "this2" {
  ...
  provisioner "local-exec" {
    when    = create
    command = "gcloud spanner backups add-iam-policy-binding ${var.spanner-bu} --instance=${var.spanner-i} --member='serviceAccount:${var.sa}@${var.project}.iam.gserviceaccount.com' --role='roles/spanner.backupAdmin'"
  }
