...
def auto_approve(path, verbosity=False, up=False, remove=False):
    command = f"cd {path} ; terraform {'apply' if up else 'destroy'} -auto-approve {'> /dev/null' if not verbosity else ''}"
...
def green_red_infrastructures_up_down(path, infra_color, up=False, down=False, verbosity=False, remove=False):
    try:
        ...
        if path is not None:
            if up:
                command = f"cd {path} ; terraform init {'> /dev/null' if not verbosity else ''}; \
                          terraform validate {'> /dev/null' if not verbosity else ''}; \
                          terraform fmt {'> /dev/null' if not verbosity else ''}"
